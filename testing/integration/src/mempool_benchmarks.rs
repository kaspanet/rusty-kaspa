...
use keryx_consensus_core::{constants::SOMPI_PER_KASPA, network::NetworkType, tx::Transaction};
use keryx_core::{debug, info};
use keryx_notify::{
    listener::ListenerId,
    scope::{NewBlockTemplateScope, Scope},
};
use keryx_rpc_core::{Notification, RpcError, api::rpc::RpcApi};
...
async fn bench_bbt_latency() {
    ...
    */
    ...
    let submit_block_pool_tasks = submit_block_pool.start(|c, block| async move {
        let _sw = keryx_core::time::Stopwatch::<500>::with_threshold("sb");
    ...
    let submit_tx_pool_tasks = submit_tx_pool.start(|c, (i, tx)| async move {
        match c.submit_transaction(tx.as_ref().into(), false).await {
            ...
            Err(RpcError::General(msg)) if msg.contains("orphan") => {
                keryx_core::warn!("\n\n\n{msg}\n\n");
                keryx_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                return true;
            }
            Err(e) => panic!("{e}"),
        }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = keryx_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                keryx_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        keryx_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                keryx_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        keryx_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        keryx_core::warn!("Tx sender task, waiting for mempool to drain..");
        loop {
            if !exec.load(Ordering::Relaxed) {
                break;
            }
        ...
        exec.store(false, Ordering::Relaxed);
        keryx_core::warn!("Tx sender task exited");
    });
