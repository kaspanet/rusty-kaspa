...
use keryx_core::{info, task::tick::TickService, trace};
use keryx_math::Uint256;
use keryx_notify::scope::VirtualDaaScoreChangedScope;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), keryx_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}
...
async fn utxos_changed_subscriptions_sanity_check() {
    ...
    keryx_core::log::try_init_logger(
        "INFO, keryx_core::time=debug, keryx_rpc_core=debug, keryx_grpc_client=debug, keryx_notify=info, keryx_notify::address::tracker=debug, keryx_notify::listener=debug, keryx_notify::subscription::single=debug, keryx_mining::monitor=debug, keryx_testing_integration::subscribe_benchmarks=trace",
    );
    ...
    // Fold-up
    //
    keryx_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    keryx_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    keryx_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package kaspa-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=22610 --p2p=22611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    keryx_core::log::try_init_logger(
        "INFO, keryx_core::core=trace, keryx_core::time=debug, keryx_rpc_core=debug, keryx_grpc_client=debug, keryx_notify=info, keryx_notify::address::tracker=debug, keryx_notify::listener=debug, keryx_notify::subscription::single=debug, keryx_mining::monitor=debug, keryx_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    keryx_core::panic::configure_panic();
