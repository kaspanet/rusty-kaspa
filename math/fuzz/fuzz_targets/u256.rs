...
where
    T: Fn(Uint256, Uint256) -> (Uint256, bool),
...
fn overflowing_shl_bigint(a: &BigUint, b: u32, mask: &BigUint) -> (BigUint, bool) {
    let overflow = b >= 256;
    let b = b % 256; // mask shift to be within bounds
    ((a << b) & mask, overflow)
}
...
fuzz_target!(|data: &[u8]| {
    ...
    assert_op_with_overflow(&mut data, Uint256::overflowing_add, |a, b| biguint_overflowing_op(a, b, mask, Add::add), true);
    // Full Overflow multiplication
    assert_op_with_overflow(&mut data, Uint256::overflowing_mul, |a, b| biguint_overflowing_op(a, b, mask, Mul::mul), true);
    // Full Overflow subtraction
    assert_op_with_overflow(&mut data, Uint256::overflowing_sub, |a, b| overflowing_sub_bigint(a, b, mask), true);
    // Full division
