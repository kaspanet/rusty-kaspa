...
use utxoindex::{
    api::UtxoIndexApi, model::CirculatingSupply, notify::UtxoIndexNotification,
    test_helpers::struct_builders::generate_random_script_public_key, UtxoIndex, VirtualChangeEmulator,
};

/// This test is and uses an quick and dirty ad hoc, ineffecient and preliminary testing platform, utilizing a custom virtual change emulator.
...
async fn test_utxoindex() {
    ...
    for item in consensus_utxos.into_iter() {
        let tx_outpoint = item.0;
        let utxo_entry = item.1.clone();
    ...
    virtual_change_emulator.change_virtual_state(rng.gen_range(200..=200), rng.gen_range(200..=200), rng.gen_range(32..=32));

    let now = Instant::now();
    let results = utxoindex.update(
        virtual_change_emulator.virtual_state.virtual_utxo_diff.clone(),
        virtual_change_emulator.virtual_state.virtual_parents,
    );
    println!("updating 10_000 * 2, took {} ms", now.elapsed().as_millis()); //ad-hoc benchmark

    match results.expect("expected notfication iterator").next().expect("expected notification").as_ref() {
        UtxoIndexNotification::UtxoChanges(utxo_changed) => {
            let mut i = 0;
...
//test just to make sure futher integration tests for rpc-core, requiring conversion, works fine.
#[test]
async fn test_script_public_key_struct_builder() {
    let script_public_key = generate_random_script_public_key();
    let script_public_key_address = script_public_key.to_p2pk_schnorr_address(Prefix::Simnet).expect("expected address");
    let script_public_key_address_string = String::try_from(script_public_key_address).expect("expected decoding to sting"); //this is actual check.
    println!("{}", script_public_key_address_string);
}
...
//if this assertion ever fails, need to rework script public key buckets.
#[test]
async fn test_script_vector_length_encodable() {
