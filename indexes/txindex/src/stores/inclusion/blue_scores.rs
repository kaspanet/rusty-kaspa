use crate::model::bluescore_refs::BlueScoreIncludingRefData;
...
impl From<BlueScoreRefKey> for BlueScoreIncludingRefData {
    #[inline(always)]
    fn from(key: BlueScoreRefKey) -> Self {
        BlueScoreIncludingRefData { including_blue_score: key.get_blue_score(), tx_id: key.get_txid() }
    }
}
...
pub trait TxIndexIncludingBlueScoreRefReader {
    fn get_blue_score_refs(
        &self,
        blue_score_range: impl RangeBounds<u64>,
        limit: Option<usize>, // if some, Will stop after limit is reached
    ) -> StoreResult<BlueScoreRefDataResIter<impl Iterator<Item = BlueScoreIncludingRefData>>>;

    fn get_lowest_blue_score_ref(&self) -> StoreResult<Option<BlueScoreIncludingRefData>>;
}

pub trait TxIndexIncludingBlueScoreRefStore: TxIndexIncludingBlueScoreRefReader {
...
// --- implementations ---
#[derive(Clone)]
pub struct DbTxIndexIncludingBlueScoreRefStore {
    db: Arc<DB>,
    access: CachedDbAccess<BlueScoreRefKey, ()>, // No value, only keys matter
}

impl DbTxIndexIncludingBlueScoreRefStore {
    pub fn new(db: Arc<DB>, cache_policy: CachePolicy) -> Self {
        Self {
            db: Arc::clone(&db),
            access: CachedDbAccess::new(db, cache_policy, DatabaseStorePrefixes::IncludingBlueScoreRefs.into()),
        }
    }
}

impl TxIndexIncludingBlueScoreRefReader for DbTxIndexIncludingBlueScoreRefStore {
    ...
    fn get_lowest_blue_score_ref(&self) -> StoreResult<Option<BlueScoreIncludingRefData>> {
        Ok(self.access.iterator().next().map(|res| BlueScoreIncludingRefData::from(BlueScoreRefKey::from(res.unwrap().0))))
    }
}

impl TxIndexIncludingBlueScoreRefStore for DbTxIndexIncludingBlueScoreRefStore {
    fn add_blue_score_refs<I>(&mut self, writer: &mut impl DbWriter, to_add_data: BlueScoreRefIter<I>) -> StoreResult<()>
    where
        I: Iterator<Item = BlueScoreRefTuple>,
    {
...
mod tests {
    ...
    fn test_blue_score_refs_key_roundtrip() {
        ...
        let blue_score_ref_data = BlueScoreIncludingRefData::from(key.clone());
        assert_eq!(blue_score, blue_score_ref_data.including_blue_score);
    ...
    fn test_get_blue_score_refs_store() {
        ...
        let mut store = DbTxIndexIncludingBlueScoreRefStore::new(Arc::clone(&txindex_db), CachePolicy::Empty);
        ...
        // Test retrieval
        let results = store.get_blue_score_refs(100u64..300u64, None).unwrap().collect::<Vec<BlueScoreIncludingRefData>>();
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].including_blue_score, to_add[0].0);
        assert_eq!(results[0].tx_id, to_add[0].1);
        assert_eq!(results[1].including_blue_score, to_add[1].0);
