...
pub fn reindex_virtual_changed_notification<'a>(
    ...
    ReindexedVirtualChangedState {
        ...
        sink_blue_score: notification
            .added_chain_blocks_acceptance_data
            .last()
            .and_then(|data| data.last())
            .map(|mbad| mbad.accepting_blue_score)
            .unwrap(),
        reindexed_mergeset_state: notification
            ...
            .enumerate()
            .flat_map(|(mergeset_index, (accepting_hash, added_acceptance_vec))| {
                added_acceptance_vec
                    .iter()
                    .map(move |mbad| reindex_mergeset_acceptance_data(accepting_hash, mergeset_index as MergesetIndexType, mbad))
            })
            .collect(),
    }
}
...
pub fn reindex_mergeset_acceptance_data<'a>(
    ...
    let txs_iter = mergeset_block_acceptance.accepted_transactions.iter().map(move |accepted_tx_entry| {
        (accepted_tx_entry.transaction_id, mergeset_block_acceptance.accepting_blue_score, *accepting_block_hash, mergeset_index)
    });

    let blue_score_refs_iter = mergeset_block_acceptance
        .accepted_transactions
        .iter()
        .map(move |accepted_tx_entry| (mergeset_block_acceptance.accepting_blue_score, accepted_tx_entry.transaction_id));
