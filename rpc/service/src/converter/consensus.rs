...
impl ConsensusConverter {
    ...
    pub fn get_transaction_input_with_verbosity(
        ...
        Ok(RpcOptionalTransactionInput {
            ...
            sig_op_count: if verbosity.include_sig_op_count.unwrap_or(false) { Some(input.mass.sig_op_count().unwrap_or(0)) } else { Default::default() },
            compute_mass: if verbosity.include_sig_op_count.unwrap_or(false) { Some(input.mass.compute_mass().unwrap_or(0)) } else { Default::default() }, // TODO: consider having a separate flag for compute_mass
