...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. This is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_reward_info_call(
        ...
        let queried_blue_index =
            merging_chain_block_ghostdag.coloring_ghostdag.mergeset_blues.iter().position(|hash| *hash == request.hash).ok_or_else(
                || {
        ...
        if merging_chain_block_ghostdag.coloring_ghostdag.selected_parent == request.hash {
            ...
            // see CoinbaseManager::expected_coinbase_transaction
            if queried_coinbase.outputs.len() == queried_ghostdag.coloring_ghostdag.mergeset_blues.len() + 1 {
