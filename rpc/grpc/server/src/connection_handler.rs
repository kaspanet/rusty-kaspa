...
use kaspa_utils_tower::{
    ...
    middleware::{measure_request_body_size_layer, BodyExt, CountBytesBody, MapResponseBodyLayer},
};
use std::fmt::Debug;
use std::{
    pin::Pin,
    ...
    pub(crate) fn serve(&self, serve_address: NetAddress) -> OneshotSender<()> {
        ...
        let server_handle = tokio::spawn(async move {
            ...
            let serve_result = TonicServer::builder()
                ...
                .layer(measure_request_body_size_layer(bytes_rx, |b|  b.map_err(|e| tonic::Status::from_error(Box::new(e))).boxed_unsync()))
