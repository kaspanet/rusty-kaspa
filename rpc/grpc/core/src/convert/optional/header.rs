...
mod tests {
    ...
    fn rejects_out_of_range_optional_header_fields() {
        let wire = protowire::RpcOptionalHeader { version: Some(u32::from(u16::MAX).saturating_add(1)), ..Default::default() };
        assert!(matches!(kaspa_rpc_core::RpcOptionalHeader::try_from(&wire), Err(RpcError::IntConversionError(_))));
