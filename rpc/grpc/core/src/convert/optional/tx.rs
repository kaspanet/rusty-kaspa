...
mod tests {
    ...
    fn test_rpc_optional_transaction_compute_budget_roundtrip() {
        let tx = RpcOptionalTransaction {
            ...
            inputs: vec![kaspa_rpc_core::RpcOptionalTransactionInput {
                previous_outpoint: Some(kaspa_rpc_core::RpcOptionalTransactionOutpoint {
                    transaction_id: Some(kaspa_consensus_core::Hash::from_bytes([2; 32])),
                    index: Some(0),
                }),
    ...
    fn rejects_out_of_range_optional_transaction_fields() {
        let wire = protowire::RpcOptionalTransaction { version: Some(u32::from(u16::MAX).saturating_add(1)), ..Default::default() };
        assert!(matches!(RpcOptionalTransaction::try_from(&wire), Err(RpcError::IntConversionError(_))));

        let wire_input =
            protowire::RpcOptionalTransactionInput { sig_op_count: Some(u32::from(u8::MAX).saturating_add(1)), ..Default::default() };
        assert!(matches!(kaspa_rpc_core::RpcOptionalTransactionInput::try_from(&wire_input), Err(RpcError::IntConversionError(_))));

        let wire_input = protowire::RpcOptionalTransactionInput {
            compute_budget: Some(u32::from(u16::MAX).saturating_add(1)),
            ..Default::default()
        };
        assert!(matches!(kaspa_rpc_core::RpcOptionalTransactionInput::try_from(&wire_input), Err(RpcError::IntConversionError(_))));
    }
