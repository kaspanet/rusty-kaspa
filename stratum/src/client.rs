...
use crate::protocol::{parse_message, create_error_response, create_success_response, create_notification, StratumRequest, StratumResponse, StratumNotification, MiningSubscribeParams, MiningAuthorizeParams, MiningSubmitParams};
...
impl StratumClient {
    ...
    async fn handle_subscribe_static(state: &Arc<RwLock<MinerState>>, request: StratumRequest, client_addr: SocketAddr, default_difficulty: f64) -> Result<StratumResponse, StratumError> {
        let params = MiningSubscribeParams::try_from(&request)
            .map_err(StratumError::Protocol)?;
    ...
    async fn handle_authorize_static(
        ...
        let params = MiningAuthorizeParams::try_from(&request)
            .map_err(StratumError::Protocol)?;
        ...
        let kaspa_address = Address::try_from(address_with_prefix.as_str())
            .map_err(|e| {
        ...
        let address_clean = address_with_prefix
            .trim_start_matches("kaspa:")
            .trim_start_matches("kaspatest:")
            .to_string();
    ...
    async fn handle_submit_static(
        ...
        let params = MiningSubmitParams::try_from(&request)
            .map_err(StratumError::Protocol)?;

        log::debug!("Received mining.submit from {} - Job: {}, Nonce: {}", 
                   client_addr, params.job_id, params.nonce);
        ...
        if !is_subscribed {
            ...
            return Ok(create_error_response(
                request.id,
                StratumError::NotSubscribed.code(),
        ...
        if !is_authorized {
            ...
            return Ok(create_error_response(
                request.id,
                StratumError::UnauthorizedWorker.code(),
        ...
        if !job_exists {
            log::warn!("[REJECTED] Stale job - Stratum submit from {} - Job not found: {} (job may have expired)", 
                       client_addr, params.job_id);
            return Ok(create_error_response(
                request.id,
                StratumError::JobNotFound.code(),
        ...
        if !extra_nonce.is_empty() {
            ...
            if work_to_parse.len() <= extranonce2_len {
                ...
                log::debug!("[NONCE] Combined extranonce {} with work {} -> {}", 
                          extra_nonce, params.nonce, work_to_parse);
            } else {
                log::debug!("[NONCE] Work length {} > extranonce2_len {}, using work as-is", 
                          work_to_parse.len(), extranonce2_len);
            }
        } else {
        ...
        let nonce = match encoding {
            Encoding::Bitmain => {
                ...
                let parsed = work_to_parse.parse::<u64>()
                    .map_err(|_| {
                    ...
                    // Rate limiting check (like JS pool)
                    {
                        ...
                        if elapsed_ms > 1000 {
                            ...
                            if messages_per_second > MAX_MESSAGES_PER_SECOND * RATE_LIMIT_THRESHOLD_MULTIPLIER {
                                log::warn!("Rate limit exceeded for {} ({} messages in {}ms) - disconnecting", 
                                          client_addr_for_handle, state_guard.message_count, elapsed_ms);
                                drop(state_guard);
                    ...
                    match parse_message(line.as_bytes()) {
                        ...
                        Ok(request) => {
                            ...
                            let response = match method.as_str() {
                                "mining.subscribe" => {
                                    let response = Self::handle_subscribe_static(&state, request.clone(), client_addr_for_handle, default_difficulty_for_handle).await?;
                                    ...
                                    if let Some(server_job) = server_job_opt {
                                        ...
                                        log::info!("Sent job {} immediately after subscribe to {}", server_job.id, client_addr_for_handle);
                                    } else {
                                        log::warn!("No job available to send immediately after subscribe to {}", client_addr_for_handle);
                                    }
                                    
                                    Ok(response)
                                }
                                "mining.authorize" => {
                                    Self::handle_authorize_static(&state, request, client_addr_for_handle, Some(miner_addresses_for_handle.clone())).await
                                }
                                "mining.submit" => {
                                    Self::handle_submit_static(&state, &jobs, &submission_tx_for_handle, client_addr_for_handle, request).await
                                }
