...
impl StratumServer {
    ...
    pub async fn start(&self) -> Result<(), StratumError> {
        ...
        let submission_rx = self.submission_rx.lock().take()
            .ok_or_else(|| StratumError::Unknown("Submission receiver already taken".to_string()))?;
    ...
    async fn job_distribution_loop(
        ...
        loop {
            ...
            let miner_address = {
                ...
                miner_addresses_read.values().next().cloned().unwrap_or_else(|| {
                    Address::new(
                        Prefix::Mainnet,
                        Version::PubKey,
                        &[0u8; 32],
            ...
            if miner_addresses.read().is_empty() {
                log::debug!("No authorized miners yet - using placeholder address for block template (will update once miners authorize)");
            } else {
    ...
    fn create_job_from_template(
        ...
        *counter += 1;
        ...
        log::info!("Template pre-PoW hash: {} (length: {}), nonce: {}, timestamp: {}, bits: {}, calculated difficulty: {}", 
                   hash_hex, hash_hex.len(), template.block.header.nonce, template.block.header.timestamp, 
                   template.block.header.bits, calculated_difficulty);
        ...
        log::info!("Job {} - prePoWHash+timestamp: {} (length: {}, expected: 80), difficulty: {}", 
                  job_id, header_hash, header_hash.len(), job_difficulty);
    ...
    async fn block_submission_loop(
        ...
        let mut last_cleanup = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs();
        ...
        while let Some(submission) = submission_rx.recv().await {
            ...
            if FIRST_SUBMISSION.swap(false, std::sync::atomic::Ordering::Relaxed) {
                log::info!("Processing block submissions - first submission received from {} (Job: {}, Nonce: 0x{:x})", 
                          submission.client_addr, submission.job_id, submission.nonce);
            } else {
                log::debug!("Received block submission from {} - Job: {}, Nonce: 0x{:x}", 
                          submission.client_addr, submission.job_id, submission.nonce);
            }
            ...
            let nonce_key = (submission.job_id.clone(), submission.nonce);
            {
                ...
                if seen.contains(&nonce_key) {
                    ...
                    log::debug!("[REJECTED] Duplicate share - job {} nonce 0x{:x} already seen", 
            ...
            let job = match jobs_read.get(&submission.job_id) {
                ...
                None => {
                    ...
                    log::debug!("[REJECTED] Stale job - received submission for unknown job: {} from {} (job may have expired)", submission.job_id, submission.client_addr);
                    drop(jobs_read);
                    continue;
                }
            };
            ...
            log::debug!("[BLOCK] Reconstructing block for job {} with nonce 0x{:x}", 
                      submission.job_id, submission.nonce);
            ...
            if current_time > template_timestamp {
                ...
                if template_age > 10 {
                    ...
                    log::debug!("[REJECTED] Stale template - job {} is {} seconds old (max 10s) - skipping submission", 
                              submission.job_id, template_age);
            ...
            if pow_passed {
                log::info!("[BLOCK FOUND] Job {} - Nonce: 0x{:x}, PoW value: {}, Network target passed! Submitting to consensus...", 
                          submission.job_id, submission.nonce, pow_value);
            } else {
                log::debug!("[PoW] Job {} - Nonce: 0x{:x}, PoW value: {}, Pool target: {}, Network passed: {}", 
                          submission.job_id, submission.nonce, pow_value, pool_target, pow_passed);
            }
            ...
            if !meets_pool_difficulty {
                ...
                let should_reduce_difficulty = {
                    ...
                    if !vardiff_states_write.contains_key(&submission.client_addr) {
                        vardiff_states_write.insert(submission.client_addr, ClientVardiffState {
                            last_share: 0,
                            last_difficulty_change: now_ms,
                        ...
                        log::debug!("Initialized vardiff state for {} with difficulty {} (from job)", 
                                  submission.client_addr, job.difficulty);
                    }
                    ...
                    if let Some(vardiff_state) = vardiff_states_write.get_mut(&submission.client_addr) {
                        ...
                        if job.difficulty > vardiff_state.current_difficulty {
                            log::debug!("Updating vardiff state difficulty from {} to {} (job difficulty is higher)", 
                        ...
                        if vardiff_state.rejected_share_count >= 50 {
                            ...
                            let final_diff = if vardiff_config.clamp_pow2 {
                                Self::clamp_to_power_of_2(new_diff)
                            } else {
                                new_diff
                            };
                            ...
                            log::warn!(
                                ...
                                submission.client_addr, old_diff, final_diff
                            );
                ...
                log::debug!("[REJECTED] Share from {} does not meet pool difficulty (job: {}, difficulty: {}, PoW value: {}, target: {})", 
                          submission.client_addr, submission.job_id, job.difficulty, pow_value, pool_target);
                continue;
            }
            ...
            if vardiff_config.enabled {
                ...
                if share_count == 1 || share_count % 100 == 0 {
                    log::info!("Share accepted from {} (job: {}, difficulty: {}, PoW: {}, total shares: {})", 
                              submission.client_addr, submission.job_id, job.difficulty, pow_value, share_count);
                }
            ...
            if !pow_passed {
                log::debug!("Share from {} meets pool difficulty but not network difficulty (PoW value: {}) - not submitting to consensus", 
                           submission.client_addr, pow_value);
                continue;
            }
            
            // Block meets network PoW difficulty - submit to consensus
            log::info!("Block from {} meets network PoW difficulty (PoW value: {}) - submitting to consensus", 
                      submission.client_addr, pow_value);
            ...
            let block_hash_short = if block_hash_str.len() >= 16 {
                block_hash_str[..16].to_string()
            } else {
            ...
            tokio::spawn(async move {
                ...
                match (block_result, virtual_state_result) {
                    (Ok(_), Ok(_)) => {
                        ...
                        log::info!("🎉 BLOCK FOUND! Hash: {}... - Mined by client {}", 
                                  block_hash_short, client_addr);
                        log::info!("✓ Block {} successfully validated and inserted into chain by client {}", 
                                  block_hash_full, client_addr);
                    }
                    (Ok(_), Err(e)) => {
    ...
    fn send_difficulty_update(
        ...
        if let Some(notification_tx) = clients_read.get(client_addr) {
            let difficulty_notification = create_notification(
                ...
                vec![Value::Number(
                    serde_json::Number::from_f64(difficulty)
    ...
    /// 1. Once after subscribe (initial difficulty)
    /// 2. When vardiff adjusts difficulty
    ...
    fn send_job_notification(
        ...
        let job_notification = create_notification(
            ...
            vec![
                ...
                Value::String(job.header_hash.clone()),
            ],
        );
        log::info!("Sending mining.notify for job {} - hash+timestamp: {} (length: {}, template_hash: {}, difficulty: {})", 
                  job.id, job.header_hash, job.header_hash.len(), job.template_hash, job.difficulty);
        tx.send(job_notification.clone())
            .map_err(|_| StratumError::Protocol("Failed to send job".to_string()))?;
