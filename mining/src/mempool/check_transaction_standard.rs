...
mod tests {
    ...
    fn test_calc_min_required_tx_relay_fee() {
        ...
        for test in tests.iter() {
            for net in NetworkType::iter() {
                ...
                let mut config = Config::build_default(params.prior_target_time_per_block, false, params.max_block_mass);
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        for test in tests {
            for net in NetworkType::iter() {
                let params: Params = net.into();
                let config = Config::build_default(params.prior_target_time_per_block, false, params.max_block_mass);
                let counters = Arc::new(MiningCounters::default());
                let mempool = Mempool::new(Arc::new(config), counters);
