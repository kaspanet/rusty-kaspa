...
use crate::{
    ...
    zk_precompiles::{
        ...
        risc0::{groth16::R0Groth16Precompile, succinct::R0SuccinctPrecompile}, tags::ZkTag,
    },
};
...
pub fn verify_zk(dstack: &mut Stack) -> Result<(), TxScriptError> {
    ...
    let tag = ZkTag::try_from(tag_bytes[0])
        .map_err(|e| TxScriptError::ZkIntegrity(e.to_string()))?;
    
    // Matcth the tag and verify the proof accordingly
    match tag {
        ZkTag::R0Groth16 => R0Groth16Precompile::verify_zk(dstack)
            .map_err(|e| TxScriptError::ZkIntegrity(e.to_string())),
        ZkTag::R0Succinct => R0SuccinctPrecompile::verify_zk(dstack)
            .map_err(|e| TxScriptError::ZkIntegrity(e.to_string())),
    }
}
...
pub fn compute_zk_sigop_cost(tag: u8) -> u32 {
    ZkTag::try_from(tag)
        .map(|t| t.sigop_cost())
        .unwrap_or(ZkTag::max_cost()) // Default to highest cost for unknown tags
}