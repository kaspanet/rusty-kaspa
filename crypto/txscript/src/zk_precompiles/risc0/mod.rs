...
impl Receipt {
    ...
    pub fn new(inner: InnerReceipt, journal: Vec<u8>) -> Self {
        let metadata = ReceiptMetadata { verifier_parameters: inner.verifier_parameters() };
        Self { inner, journal: Journal::new(journal), metadata }
    }
    ...
    pub fn verify_integrity_with_context(&self, ctx: &VerifierContext) -> Result<(), VerificationError> {
        ...
        let claim = maybe_pruned_claim.as_value().map_err(|_| VerificationError::ReceiptFormatError)?;
...
impl<Claim> FakeReceipt<Claim> {
    ...
    where
        Claim: risc0_binfmt::Digestible,
    {
        FakeReceipt { claim: MaybePruned::Pruned(self.claim.digest()) }
    }
}
...
use crate::zk_precompiles::risc0::{claim::maybe_pruned::MaybePruned, sha::{DIGEST_BYTES,Digest}};
    #[test]
    fn mangled_version_info_should_error() {
        let mut mangled_receipt = Receipt::new(InnerReceipt::Fake(FakeReceipt { claim: MaybePruned::Pruned(Digest::ZERO) }), vec![]);
        let ones_digest = Digest::from([1u8; DIGEST_BYTES]);
        mangled_receipt.metadata.verifier_parameters = ones_digest;

        assert_eq!(mangled_receipt.verify(Digest::ZERO).err().unwrap(), VerificationError::ReceiptFormatError);
        assert_eq!(
            mangled_receipt.verify_with_context(&Default::default(), Digest::ZERO).err().unwrap(),
            VerificationError::ReceiptFormatError
        );
        assert_eq!(
            mangled_receipt.verify_integrity_with_context(&Default::default()).err().unwrap(),
            VerificationError::ReceiptFormatError
        );
    }
