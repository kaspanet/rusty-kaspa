...
/// offchain so the verifier fragment doesn't have to deserialize/serialize the
/// key at build time. `compressed_vk_matches_uncompressed_vk` pins it to the
/// canonical [`R0_SERIALIZED_UNCOMPRESSED_VK`][crate::zk_to_script::builder::R0_SERIALIZED_UNCOMPRESSED_VK].
