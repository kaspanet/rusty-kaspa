...
The _basic-http-server_ will serve on port 4000 by default, so open your web browser and load http://localhost:4000
...
The fastest way to run Rusty Kaspa is using Docker. This method avoids complex build dependencies and provides a consistent environment across different platforms.
...
2. **Start mainnet node with wRPC proxy:**
...
3. **Access your node:**
   - **gRPC**: `localhost:16110` (direct node access)
   - **wRPC Borsh**: `localhost:17110` (for wallets - recommended)
   - **P2P**: `localhost:16111` (peer-to-peer)
...
The Docker setup uses environment variables for configuration. Copy the example file and customize:
...
- **Permission issues on macOS**: The containers run as root to avoid macOS filesystem permission limitations
- **Port conflicts**: Ensure no other services are using the required ports
- **Build issues**: Use `./docker-start.sh -b mainnet` to rebuild images
