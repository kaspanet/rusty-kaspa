...
`components/kaspa-stratum-bridge`
...
```bash
cargo run -p kaspa-stratum-bridge --release --bin stratum-bridge -- --config components/kaspa-stratum-bridge/config.yaml --node-mode inprocess --node-args="--utxoindex --rpclisten=127.0.0.1:16110 --rpclisten-borsh=127.0.0.1:17110"
```
