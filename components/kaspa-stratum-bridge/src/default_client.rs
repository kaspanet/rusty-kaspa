...
static BIG_JOB_REGEX: LazyLock<Regex> = LazyLock::new(|| {
...
pub fn default_handlers() -> std::collections::HashMap<String, crate::stratum_listener::EventHandler> {
    ...
    handlers.insert(
        ...
        Arc::new(|ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            Box::pin(async move {
                handle_subscribe(ctx, event, None).await
    ...
    handlers.insert(
        ...
        Arc::new(|ctx: Arc<StratumContext>, event: JsonRpcEvent| {
            ...
            let event = event.clone();
            Box::pin(async move {
                handle_extranonce_subscribe(ctx, event).await
            }) as std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), Box<dyn std::error::Error + Send + Sync>>> + Send>>
        }) as crate::stratum_listener::EventHandler,
    );
...
pub async fn handle_subscribe(
    ...
    tracing::debug!("[SUBSCRIBE] Client info - app: '{}', extranonce: '{}', addr: {}", 
                   remote_app, extranonce, ctx.remote_addr);
...
pub async fn handle_authorize(
    ...
    tracing::debug!("[AUTHORIZE] Final parsed - address: '{}', worker: '{}', canxium: '{}'", 
                  address, worker_name, canxium_address);
    ...
    *ctx.worker_name.lock() = worker_name.clone();
    ...
    if let (Some(client_handler), Some(kaspa_api)) = (client_handler, kaspa_api) {
        tracing::debug!("[AUTHORIZE] Step 3-4: Triggering immediate job send for client {} (extranonce already sent)", ctx.remote_addr);
...
async fn send_extranonce(ctx: Arc<StratumContext>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let params = if is_bitmain_flag {
        ...
        tracing::debug!("[EXTRANONCE] Bitmain extranonce: '{}' ({} bytes), extranonce2_size: {} (calculated: 8 - {} / 2)", 
                      extranonce, extranonce.len() / 2, extranonce2_size, extranonce.len());
