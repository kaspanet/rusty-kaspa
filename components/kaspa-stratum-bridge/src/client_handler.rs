...
impl ClientHandler {
    ...
    pub async fn send_immediate_job_to_client<T: KaspaApiTrait + Send + Sync + ?Sized + 'static>(
        ...
        tokio::spawn(async move {
            ...
            tracing::debug!("send_immediate_job: fetching block template for client {} (wallet: {})", client_clone.remote_addr, wallet_addr);
            ...
            tracing::debug!("[JOB CREATION] send_immediate_job: created job ID {} for client {} (counter: {}, stored IDs: {:?})", 
            ...
            tracing::debug!("[JOB] Successfully sent job ID {} to client {}", 
            ...
            tokio::spawn(async move {
                ...
                let wallet_addr_str = {
                    ...
                    if wallet_addr.is_empty() {
                        ...
                        tracing::debug!("new_block_available: client {} has no wallet address yet, skipping", client_clone.remote_addr);
                        return;
                    }
                    wallet_addr.clone()
                };
                
                tracing::debug!("new_block_available: fetching block template for client {} (wallet: {})", client_clone.remote_addr, wallet_addr_str);
                ...
                let block = match template_result {
                    Ok(block) => {
                        tracing::debug!("new_block_available: successfully fetched block template for client {}", client_clone.remote_addr);
                        block
                    },
                    Err(e) => {
                        if e.to_string().contains("Could not decode address") {
                ...
                let counter_after = state.current_job_counter();
                let stored_ids = state.get_stored_job_ids();
                tracing::debug!("[JOB CREATION] new_block_available: created job ID {} for client {} (counter: {}, stored IDs: {:?})", 
                              job_id, client_clone.remote_addr, counter_after, stored_ids);
                ...
                if let Err(e) = send_result {
                    if e.to_string().contains("disconnected") {
                        ...
                        tracing::error!("new_block_available: failed to send job {} to client {}: {}", 
                    ...
                    tracing::debug!("new_block_available: successfully sent job ID {} to client {}", 
                                  job_id, client_clone.remote_addr);
                }
            });
        }
