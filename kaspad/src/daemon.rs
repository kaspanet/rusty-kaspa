...
do you confirm? (answer y/n or pass --yes to the Kaspad command line to confirm all interactive questions)";
    ...
    let mut block_template_cache_lifetime: Option<u64> = None;
    #[cfg(feature = "devnet-prealloc")]
    if config.block_template_cache_lifetime.is_some() {
        block_template_cache_lifetime = config.block_template_cache_lifetime;
    }
    let mining_manager = MiningManagerProxy::new(Arc::new(MiningManager::new(
        config.target_time_per_block,
        false,
        config.max_block_mass,
        block_template_cache_lifetime,
    )));
