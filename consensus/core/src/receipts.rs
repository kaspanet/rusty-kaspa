...
pub struct TxReceipt {
    ...
    pub posterity_context: PosterityReceiptContext,
    pub lane_key: Hash,
}
...
impl TxReceipt {
    ...
    pub fn verify_receipt(&self, expected_posterity_sqc: Hash) -> bool {
        ...
        matches!(self.posterity_sqc(posterity_lane_tip), Some(sqc) if sqc == expected_posterity_sqc)
    }
    ...
    pub fn posterity_lane_tip(&self, accepting_blk_lane_tip: Hash) -> Hash {
        ...
        for update in &self.lane_tip_updates_to_posterity {
    ...
    pub fn posterity_sqc(&self, posterity_lane_tip: Hash) -> Option<Hash> {
        let posterity_lane_leaf = self.posterity_lane_leaf(posterity_lane_tip);
        let posterity_active_lanes_root = self.posterity_active_lanes_root(posterity_lane_leaf)?;
...
impl TxReceipt {
    // Internal helpers
    fn posterity_lane_leaf(&self, posterity_lane_tip: Hash) -> Hash {
        kaspa_seq_commit::hashing::smt_leaf_hash(&SmtLeafInput {
            lane_tip: &posterity_lane_tip,
            blue_score: self.posterity_context.lane_last_blue_score,
        })
    }

    fn posterity_active_lanes_root(&self, posterity_lane_leaf: Hash) -> Option<Hash> {
        self.posterity_context
