...
fn main() {
    ...
    if env::var("RUSTY_KASPA_NO_COMMIT_HASH").is_err() {
        if let Some(GitHead { head_path, head_ref_path, commit_hash }) = try_git_head() {
            println!("cargo::rerun-if-changed={head_path}");
            println!("cargo::rerun-if-changed={head_ref_path}");
            println!("cargo:rustc-env=RUSTY_KASPA_GIT_COMMIT_HASH={commit_hash}");
        }
    }
}
        ...
        if head_path.is_file() {
            ...
            if head.starts_with("ref: ") {
                ...
                if head_ref_path.is_file() {
                    if let Ok(commit_hash) = std::fs::read_to_string(&head_ref_path) {
                        return Some(GitHead {
