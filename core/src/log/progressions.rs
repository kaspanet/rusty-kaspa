...
static RUNNING_REPORTER: Lazy<Option<ProgressBar>> = Lazy::new(|| if *MULTI_PROGRESS_BAR_ACTIVE{ 
...
pub static INFO_REPORTER: Lazy<Option<ProgressBar>> = Lazy::new(|| if *MULTI_PROGRESS_BAR_ACTIVE { 
    let heading = "{prefix:!.116.bold}:  {msg:!.249} \n\n";
    let style = ProgressStyle::default_spinner()
        .template(heading)
        .expect("expected default spinner template to be valid");
...
pub static WARN_REPORTER: Lazy<Option<ProgressBar>> = Lazy::new(|| if *MULTI_PROGRESS_BAR_ACTIVE { 
    let heading = "{prefix:!.116.bold}:  {msg:.!.249}";
    let style = ProgressStyle::default_spinner()
        .template(heading)
        .expect("expected default spinner template to be valid");
...
pub static ERROR_REPORTER: Lazy<Option<ProgressBar>> = Lazy::new(|| if *MULTI_PROGRESS_BAR_ACTIVE { 
    let heading = "{prefix:!.116.bold}: {msg:!.249}";
    let style = ProgressStyle::default_spinner()
        .template(heading)
        .expect("expected default spinner template to be valid");
...
pub fn maybe_init_spinner(
    ...
    if let Some(mpb) = MULTI_PROGRESS_BAR.deref() {
        ...
        mpb.insert(0+6, pb.clone()); // plus one for each log reporter... (error, warn, info, debug, trace)
        return Some(pb);
    };
    None
}
...
mod test {
    ...
    fn test_inits() {
        ...
        assert!(maybe_init_progress_bar_spinner_pair(Cow::Borrowed("test"), Cow::Borrowed("test"), 0u64, true, true, true, true, true).is_some());
        ...
        unsafe {
            ...
            *val = Lazy::new(|| if MULTI_PROGRESS_BAR_ACTIVE_PROXY.load(Ordering::SeqCst) { Some(ProgressBar::new_spinner()) } else { None });
        ...
        assert!(maybe_init_progress_bar_spinner_pair(Cow::Borrowed("test"), Cow::Borrowed("test"), 0u64, true, true, true, true, true).is_none());
