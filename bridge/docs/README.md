...
**Linux AppImage (optional):** Releases may include `stratum-bridge-<version>-x86_64.AppImage`. Make it executable (`chmod +x`), then run it or use your file manager. The AppImage looks for `config.yaml` at `$XDG_CONFIG_HOME/stratum-bridge/config.yaml` (usually `~/.config/stratum-bridge/config.yaml`) when that file exists; otherwise it uses built-in defaults. Extra CLI arguments are forwarded to the bridge.
