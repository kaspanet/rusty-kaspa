...
#[derive(Clone, Debug)]
enum HttpMode {
    Dashboard { web_bind: String },
    Prometheus { instance_id: String },
}
...
async fn handle_http_request(
    ...
    if method == "GET" && path == "/metrics" {
        ...
        let metric_families = match mode {
            HttpMode::Dashboard { .. } => prometheus::gather(),
            HttpMode::Prometheus { instance_id } => filter_metric_families_for_instance(prometheus::gather(), instance_id),
        };
        let mut buf = Vec::new();
    ...
    if matches!(mode, HttpMode::Prometheus { .. }) {
    ...
    if method == "GET" && path == "/api/status" {
        ...
        let web_bind = match mode {
            HttpMode::Dashboard { web_bind } => web_bind.clone(),
            HttpMode::Prometheus { .. } => unreachable!(),
        };
...
pub async fn start_web_server_all(port: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    serve_http_loop(listener, HttpMode::Dashboard { web_bind: web_bind_for_status }).await
}
...
pub async fn start_prom_server(port: &str, instance_id: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    serve_http_loop(listener, HttpMode::Prometheus { instance_id }).await
}
...
mod tests {
    ...
    #[tokio::test]
    async fn dashboard_serves_status_and_stats() {
        set_web_status_config("127.0.0.1:16110".to_string(), 2);

        let mode = HttpMode::Dashboard { web_bind: "127.0.0.1:0".to_string() };
        ...
        let config_resp = send_request(mode, "GET /api/config HTTP/1.1\r\n\r\n").await;
        assert!(config_resp.contains("404 Not Found"));
    }

    #[tokio::test]
    async fn prometheus_mode_only_serves_exact_metrics_route() {
        let mode = HttpMode::Prometheus { instance_id: "0".to_string() };

        let metrics_resp = send_request(mode.clone(), "GET /metrics HTTP/1.1\r\n\r\n").await;
        assert!(metrics_resp.contains("200 OK"));
        assert!(metrics_resp.contains("text/plain"));
        ...
        for request in [
            ...
            "GET / HTTP/1.1\r\n\r\n",
            "GET /metrics-extra HTTP/1.1\r\n\r\n",
            "POST /metrics HTTP/1.1\r\n\r\n",
        ] {
            let response = send_request(mode.clone(), request).await;
            assert!(response.contains("404 Not Found"), "request unexpectedly succeeded: {request}");
        }
    }
}
