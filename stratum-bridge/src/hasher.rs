...
pub fn diff_to_target(diff: f64) -> BigUint {
    ...
    if use_alternative {
        ...
        tracing::debug!(
            ...
            if !alt_target.is_zero() {
                standard_target.to_f64().unwrap_or(0.0) / alt_target.to_f64().unwrap_or(1.0)
            } else {
                0.0
            }
        );
...
pub fn generate_iceriver_job_params(pre_pow_hash: &kaspa_hashes::Hash, timestamp: u64) -> String {
    ...
    let decoded_timestamp = u64::from_le_bytes([
        timestamp_le[0], timestamp_le[1], timestamp_le[2], timestamp_le[3],
        timestamp_le[4], timestamp_le[5], timestamp_le[6], timestamp_le[7],
    ]);
    ...
    tracing::debug!("[TIMESTAMP] Final job data: {} (hash: {} chars, timestamp: {} chars)", 
                  result, hash_hex.len(), timestamp_hex.len());
