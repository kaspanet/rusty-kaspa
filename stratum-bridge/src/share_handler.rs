...
impl ShareHandler {
    ...
    pub async fn handle_submit(
        ...
        tracing::debug!("[SUBMIT] ===== SHARE SUBMISSION FROM {} =====", ctx.remote_addr);
            ...
            if meets_network_target {
                ...
                match block_submit_result {
                    Ok(_response) => {
                        ...
                        log_info!("╔════════════════════════════════════════════════════════════╗");
                        log_info!("║                                                            ║");
                        log_info!("║            ✅  BLOCK ACCEPTED BY NODE  ✅                  ║");
                        log_info!("║                                                            ║");
                        log_info!("╚════════════════════════════════════════════════════════════╝");
                        log_info!("");
                    ...
                    Err(e) => {
                        ...
                        error!("{} {}", LogColors::block("[BLOCK]"), LogColors::error("✗ Block submission FAILED"));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (_, v) in stats_map.iter() {
                    ...
                    // Format uptime as seconds with decimal (e.g., "104.9870193s")
                    let uptime_secs = v.start_time.elapsed().as_secs_f64();
                    let uptime = format!("{:.7}s", uptime_secs);
                    lines.push(format!(
                        " {:<22}| {:>14} | {:>13} | {:>6} | {:>11}",
                        &*v.worker_name.lock(),
