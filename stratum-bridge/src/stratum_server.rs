//! Stratum Bridge Server - Multi-ASIC Support Architecture
//!
//! This module provides the main entry point for the Stratum Bridge server, which is designed
//! to handle multiple ASIC miner types with automatic detection and per-client configuration.
//!
//! ## ASIC-Specific Architecture
//!
//! The bridge implements sophisticated ASIC handling to support different miner types seamlessly:
//!
//! ### 1. Auto-Detection System
//! - Detects miner type from `remote_app` user-agent string in `mining.subscribe`
//! - Supported miners: IceRiver, Bitmain (GodMiner/Antminer), BzMiner, Goldshell
//! - Detection happens in `handle_subscribe` via `ClientHandler::assign_extranonce_for_miner`
//!
//! ### 2. Per-Client Extranonce Assignment
//! - **Bitmain**: extranonce_size = 0 (no extranonce, uses nonce only)
//! - **IceRiver/BzMiner/Goldshell**: extranonce_size = 2 (2-byte extranonce pool)
//! - Assignment happens automatically based on detected miner type
//! - Extranonce pool wraps at 65535 for 2-byte miners
//!
//! ### 3. Different Message Formats
//! - **Bitmain subscribe response**: `[null, extranonce, extranonce2_size]` (extranonce in response)
//! - **IceRiver/BzMiner subscribe response**: `[true, "EthereumStratum/1.0.0"]` (extranonce sent separately)
//! - **IceRiver notifications**: Minimal format (method + params only, no id/jsonrpc)
//! - **Other miners**: Standard JSON-RPC format
//!
//! ### 4. Critical Message Ordering (IceRiver)
//! For IceRiver miners, messages MUST be sent in this exact order:
//! 1. `mining.authorize` response
//! 2. `mining.set_extranonce` (if extranonce enabled)
//! 3. `mining.set_difficulty`
//! 4. `mining.notify` (job)
//!
//! This is handled in `handle_authorize` which ensures sequential sending.
//!
//! ### 5. Different Job Parameter Formats
//! - **IceRiver**: 80-character hex string (`generate_iceriver_job_params`)
//!   - Format: 64 hex chars (hash) + 16 hex chars (timestamp LE) = 80 chars
//! - **BzMiner**: Single hex string with big-endian hash (`generate_large_job_params`)
//! - **Bitmain**: Array format with separate timestamp (`generate_job_header`)
//!
//! ### 6. Immediate Job Sending
//! - After `mining.authorize`, job is sent immediately via `send_immediate_job_to_client`
//! - Does not wait for polling loop (critical for IceRiver KS2L compatibility)
//! - Ensures miners get work immediately after authorization
//!
//! ### 7. Per-Client MiningState
//! - Each client has isolated `MiningState` with its own job tracking
//! - Prevents job ID collisions between clients
//! - Supports up to MAX_JOBS (typically 8) jobs per client
//!
//! ### 8. Variable Difficulty
//! - Per-client difficulty adjustment based on share submission rate
//! - Power-of-2 clamping for ASIC compatibility (IceRiver/Bitmain)
//! - Automatic adjustment to target shares_per_min
//!
//! ## Integration Notes
//!
//! When integrated into kaspad:
//! - `listen_and_serve` is called with concrete `KaspaApi` for notification support
//! - All handlers are properly set up with `client_handler` for auto-detection
//! - `authorize` handler receives both `client_handler` and `kaspa_api` for immediate job sending
//! - Block template updates use notifications with polling fallback
//! - All ASIC-specific logic is preserved through proper handler wiring
...
///
/// All configuration options for the Stratum Bridge server.
/// Note: `extranonce_size=0` enables auto-detection per client based on miner type.
#[derive(Clone)]
...
pub async fn listen_and_serve<T: KaspaApiTrait + Send + Sync + 'static>(
    ...
    let client_handler = Arc::new(ClientHandler::new(Arc::clone(&share_handler), min_diff, extranonce_size));
