...
use crate::utxo::{
    EventConsumer, Events, PendingUtxoEntryReference, UtxoEntryId, UtxoEntryReference, UtxoProcessor, UtxoProcessorContext,
    UtxoProcessorId,
};
...
pub struct Inner {
    ...
    address_to_utxo_processor_map: DashMap<Arc<Address>, Arc<UtxoProcessor>>,
...
impl Inner {
    pub fn new(rpc: &Arc<DynRpcApi>) -> Self {
        Self {
            pending: DashMap::new(),
            address_to_utxo_processor_map: DashMap::new(),
...
impl UtxoProcessorCore {
    pub fn new(rpc: &Arc<DynRpcApi>) -> Self {
        UtxoProcessorCore { inner: Arc::new(Inner::new(rpc)) }
    }
    ...
    pub fn listener_id(&self) -> ListenerId {
        self.inner.listener_id.lock().unwrap().expect("missing listener_id in UtxoProcessorCore::listener_id()")
    }
    ...
    pub async fn clear(&self) -> Result<()> {
        self.inner.address_to_utxo_processor_map.clear();
        // TODO - clear processors?
        Ok(())
    }

    pub fn address_to_utxo_processor_map(&self) -> &DashMap<Arc<Address>, Arc<UtxoProcessor>> {
        &self.inner.address_to_utxo_processor_map
    }

    pub fn address_to_utxo_processor(&self, address: &Address) -> Option<Arc<UtxoProcessor>> {
        self.inner.address_to_utxo_processor_map.get(address).map(|v| v.clone())
    }

    pub async fn register_addresses(&self, addresses: Vec<Arc<Address>>, processor: &Arc<UtxoProcessor>) -> Result<()> {
        addresses.iter().for_each(|address| {
            self.inner.address_to_utxo_processor_map.insert(address.clone(), processor.clone());
        });
    ...
    pub async fn unregister_addresses(&self, addresses: Vec<Arc<Address>>) -> Result<()> {
        addresses.iter().for_each(|address| {
            self.inner.address_to_utxo_processor_map.remove(address);
        });
    ...
    pub async fn handle_pending(&self, current_daa_score: u64) -> Result<()> {
        ...
        let mut processors = HashMap::<UtxoProcessorId, UtxoProcessor>::default();
        for mature in mature_entries.into_iter() {
            let utxo_processor = &mature.utxo_processor;
            let entry = mature.entry;
            utxo_processor.promote(entry);

            processors.insert(utxo_processor.id(), utxo_processor.clone());
        }

        let processors = processors.values().cloned().collect::<Vec<_>>();

        for processor in processors.iter() {
            processor.update_balance().await?;
        }
    ...
    pub async fn handle_utxo_changed(&self, utxos: UtxosChangedNotification) -> Result<()> {
        ...
        for (address, entries) in added.into_iter() {
            if let Some(utxo_processor) = self.address_to_utxo_processor(&address) {
                let entries = entries.into_iter().map(|entry| entry.into()).collect::<Vec<UtxoEntryReference>>();
                utxo_processor.handle_utxo_added(entries).await?;
            } else {
        ...
        for (address, entries) in removed.into_iter() {
            if let Some(utxo_processor) = self.address_to_utxo_processor(&address) {
                ...
                utxo_processor.handle_utxo_removed(entries).await?;
            } else {
