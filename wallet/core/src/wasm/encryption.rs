...
pub fn js_sha256_hash(data: BinaryT) -> Result<HexString> {
...
pub fn js_sha256d_hash(data: BinaryT) -> Result<HexString> {
    let data = data.try_as_vec_u8()?;
...
pub fn js_argon2_sha256iv_phash(data: BinaryT, byte_length: usize) -> Result<HexString> {
    let data = data.try_as_vec_u8()?;
