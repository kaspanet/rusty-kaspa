...
impl Estimate {
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        let destination = PaymentDestination::PaymentOutputs(PaymentOutputs::from((change_address.clone(), amount_dwork)));
        let estimate = account.estimate(destination, priority_fee_dwork.into(), None, &abortable).await?;
